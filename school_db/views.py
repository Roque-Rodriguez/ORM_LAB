...
def problem_two(request):
    ...
    instructors = Instructor.objects.filter(hire_date__year__lt=2010)  ## I have changed datetime.date
...
def problem_three(request):
  ...
  courses = Course.objects.filter(instructor_id = 2) ## lopp thru courses
  for course in courses:
    print(
      f"Instructor Name: {instructors.first_name} {instructors.last_name} \n Courses:\n -{course.name}")
...
def problem_five(request):
    student =  Student.objects.create(id = 11, first_name = "Kyle", last_name = "Harwood", year = 9, gpa = 3.0)
    return complete(request)
...
def problem_six(request):
    ...
    print(f"Student's ID {student.id} Name: {student.first_name} {student.last_name} GPA: {student.gpa}")
